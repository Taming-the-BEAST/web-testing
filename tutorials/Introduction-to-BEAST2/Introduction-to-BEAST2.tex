...
TreeAnnotator is used to produce a summary tree from the posterior sample of trees using one of the available algorithms.
...
at \url{https://taming-the-beast.org/web-testing/tutorials/Introduction-to-BEAST2/}
or from Github.

\begin{framed}
\textbf{Downloading from taming-the-beast.org/web-testing}
...
\begin{figure}
    ...
    \caption{TreeAnnotator MCC tree setup}
    \label{fig:treeannot}
\end{figure}
...
\begin{framed}
	...
	Select \textbf{File -> Manage packages}
	
	Select \textbf{CCD} package in the list and click\textbf{ Install/Upgrade} (Figure \ref{fig:installCCD})
...
\begin{figure}
	\centering
	\includegraphics[width=0.800000\textwidth]{figures/installCCD0.png}
...
\begin{framed}
	Open \textbf{TreeAnotator}.
	
	Repeat all the steps in TreeAnotator as when making the MCC tree, except for \textbf{Target tree type} and \textbf{Output File}.
	
	For \textbf{Target tree type} select \textbf{MAP(CCD0)} from drop down list.
	
	Set the \textbf{Output File} to \passthrough{\lstinline!Primates.CCD0_MAP.tree!}.
\end{framed}

The setup should look as shown in Figure  \ref{fig:treeannot_CCD}. You can now run the program.

\begin{figure}
	\centering
	\includegraphics[width=0.800000\textwidth]{figures/treeannot_CCD0.png}
	\caption{TreeAnnotator CCD0 tree setup}
	\label{fig:treeannot_CCD}
\end{figure}
...
\textbf{Visualising the MCC tree}
...
\begin{figure}
    ...
    \caption{FigTree visualisation of the estimated MCC tree.}
    \label{fig:figtree_MCC}
\end{figure}

Your tree should now look something like Figure \ref{fig:figtree_MCC}. We
...
\textbf{Visualising CCD0 tree}

To visualise CCD0 tree, follow the same steps as above, but open the \passthrough{\lstinline!Primates.CCD0_MAP.tree!} at the first step. Figure \ref{fig:figtree_CCD} shows the resulting tree.

\begin{figure}
	\centering
	\includegraphics[max width=\textwidth, max height=0.9\textheight]{figures/figTree_CCD.png}
	\caption{FigTree visualisation of the estimated CCD0 tree.}
	\label{fig:figtree_CCD}
\end{figure}
...
\href{http://creativecommons.org/licenses/by/4.0/}{\includegraphics[scale=0.8]{figures/ccby.pdf}} This tutorial was written by Jūlija
